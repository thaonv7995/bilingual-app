...
def test_personal_states_are_per_user_mutually_exclusive_and_priority_sorts_first():
    ...
    try:
        ...
        assert state["isFinished"] is True
        state = update_book_state(
            "older",
            {"isPriority": True},
            current_user=user_a,
        ...
        assert state == {"isPriority": True, "onShelf": False, "isFinished": False}
    finally:
        db.close()
    ...
    assert shelf_a[0]["onShelf"] is False
    assert shelf_a[0]["isFinished"] is False
